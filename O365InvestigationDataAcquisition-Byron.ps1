...
$ClientIDEncoded = [System.Web.HttpUtility]::UrlEncode($globalConfig.InvestigationAppId)
$ClientSecret = [System.Web.HttpUtility]::UrlEncode($globalConfig.InvestigationAppSecret)
$loginURL = $globalConfig.LoginURL
$redirectUri = $globalConfig.InvestigationAppredirectUri
$redirectUriEncoded = [System.Web.HttpUtility]::UrlEncode($globalConfig.InvestigationAppredirectUri)
$tenantdomain = [System.Web.HttpUtility]::UrlEncode($globalConfig.InvestigationTenantDomain)
$TenantGUID = [System.Web.HttpUtility]::UrlEncode($globalConfig.InvestigationTenantGUID)
$resource = $globalConfig.ResourceAPI
$resourceEncoded = [System.Web.HttpUtility]::UrlEncode($globalConfig.ResourceAPI)
...
$renewbody = "grant_type=authorization_code&code=$authCode&redirect_uri=$redirectUri&resource=$resource&client_id=$ClientID&client_secret=$ClientSecret"
...
foreach ($pull in $apiFilters)
{
    ...
    $rawData += $rawRef
    $TempString = $pull.Split('&')
    Write-Host "We just pulled from the" $TempString[0].TrimStart("?contentType=") "subscription for the time period" 
    Write-Output "---"
    ...
    if ($timeLeft.TotalSeconds -lt 100) 
    {
        ...
        $oauth = Invoke-RestMethod -Method Post -Uri $loginURL/$tenantdomain/oauth2/token?api-version=1.0 -Body $renewbody; 
        $headerParams  = @{'Authorization'="$($oauth.token_type) $($oauth.access_token)"}; 
        $AuthorizationExpiration = [datetime]::Now.AddSeconds($oauth.expires_in); 
